...
They also make formatting simple through the use of Java's `DateTimeFormatter`.
...
To adhere to defensive programming practices, instead of simply adding the `Employee` to the `Shift`, we create a copy of the original `Shift` and add the employee to the copy.
Then, we replace the original shift with the copy.
. `UnapplyCommand`: Removes an `Employee` from the `UniqueEmployeeList` in the `Shift`. +
Similar to `ApplyCommand`, we create a copy of the `Shift` to be modified, remove the `Employee` from the copy, then replace the original with the copy.
